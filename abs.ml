 ...
 ***************************************)
 ...
 = fun aexp state ->
  try State.fst_of_val (State.find (absloc_of_aexp aexp state) state)
  with
  | Not_found ->
    (match aexp with
    | Int n -> Itv.int2itv n 
    | Lv lv -> State.fst_of_val (State.find (absloc_of_lv lv state) state)
    | BinOpLv (bop, lv1, lv2) -> itv_eval_bop bop (itv_eval_aexp lv1 state) (itv_eval_aexp lv2 state)
    | AHole _ -> Itv.top)
...
and sym_eval_aexp : aexp -> State.t -> Symbolic.t
= fun aexp state ->
  match aexp with
  | Int n -> Sym (SInt n)
  | Lv lv -> State.snd_of_val (State.find (absloc_of_lv lv state) state) 
  | BinOpLv (bop, lv1, lv2) -> sym_eval_bop bop (sym_eval_aexp lv1 state) (sym_eval_aexp lv2 state)
...
and absloc_of_lv : lv -> State.t -> absloc
= fun lv state -> 
  match lv with 
  | Var x -> x 
  | Arr (x,y) -> 
    let buf = Hashtbl.find dump x in 
    let idx = State.fst_of_val (abs_eval_aexp y state) in
      (match idx with
...
= fun aexp state ->
  ...
  | BinOpLv (bop, e1, e2) -> absloc_of_bop bop e1 e2 state 
  | AHole _ -> "AHole"

and absloc_of_bop : bop -> aexp -> aexp -> State.t -> absloc
= fun bop e1 e2 state ->
  match bop with
  | Plus -> absloc_of_aexp e1 state  ^ " + " ^ absloc_of_aexp e2 state
  | Minus -> absloc_of_aexp e1 state ^ " - " ^ absloc_of_aexp e2 state 
  | Mult -> absloc_of_aexp e1 state ^ " * " ^ absloc_of_aexp e2 state 
  | Div -> absloc_of_aexp e1 state ^ " / " ^ absloc_of_aexp e2 state 
  | Mod -> absloc_of_aexp e1 state ^ " % " ^ absloc_of_aexp e2 state 
...
= fun bexp state ->
  ...
  | Gt (e1, e2) ->
    (try
      let e1_itv = State.fst_of_val (abs_eval_aexp e1 state) in
      let e2_itv = State.fst_of_val (abs_eval_aexp e2 state) in
        if Itv.concrete_le e1_itv e2_itv then BFalse else
        if Itv.concrete_gt e1_itv e2_itv then BTrue
        else BTop
    with
      | Not_found -> BBot
    )
  | Lt (e1, e2) ->
    (try
      let e1_itv = State.fst_of_val (abs_eval_aexp e1 state) in
      let e2_itv = State.fst_of_val (abs_eval_aexp e2 state) in
        if Itv.concrete_ge e1_itv e2_itv then BFalse else
        if Itv.concrete_lt e1_itv e2_itv then BTrue
        else BTop
    with
      | Not_found -> BBot
    )
  | Eq (e1, e2) ->
    (try
      let e1_itv = State.fst_of_val (abs_eval_aexp e1 state) in
      let e2_itv = State.fst_of_val (abs_eval_aexp e2 state) in
        if Itv.concrete_neq e1_itv e2_itv then BFalse
        else BTop
    with
      | Not_found -> BBot
    )
...
= fun bexp state ->
  ...
  | Gt (e1, e2) -> abs_eval_bexp (Or (Lt (e1, e2), Eq (e1, e2))) state
  | Lt (e1, e2) -> abs_eval_bexp (Or (Gt (e1, e2), Eq (e1, e2))) state
  | Eq (e1, e2) -> abs_eval_bexp (Or (Lt(e1, e2), Gt(e1, e2))) state
...
let rec add_lv_state : aexp -> absval -> State.t -> State.t -> State.t
  ...
  | BTop -> 
    (match bexp with
     ...
     | Eq (e1,e2) ->
       ...
       let state' = State.add (absloc_of_aexp e1 state) (meet, e1_sym) state in
       let state'' = State.add (absloc_of_aexp e2 state) (meet, e2_sym) state' in
...
= fun bexp state ->
  ...
  | Gt (lv1,lv2) -> prune_by (Or (Lt (lv1,lv2), Eq (lv1,lv2))) state 
  | Lt (lv1,lv2) -> prune_by (Or (Gt (lv1,lv2), Eq (lv1,lv2))) state 
  | Eq (lv1,lv2) -> prune_by (Or (Gt (lv1,lv2), Lt (lv1,lv2))) state 
...
= fun lv_comps cnt cmd state ->
  ...
  | Assign (lv , AHole _) ->
    ...
    | Arr (x,y) -> State.add (absloc_of_lv lv state) (Itv.top, Symbolic.top) state
    )
  | Assign (lv, aexp) ->
    (match lv with
    | Var x -> State.add (absloc_of_lv lv state) (abs_eval_aexp aexp state) state
    | Arr (x,y) -> State.weak_add (absloc_of_lv lv state) (abs_eval_aexp aexp state) state
    )
...
let abs_run : prog -> value list -> lv list -> absval 
...
let absval_profile : prog -> absval -> absval -> unit
...
let rec hopeless : prog -> example list -> lv list -> bool
