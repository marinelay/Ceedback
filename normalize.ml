...
= fun aexp cmem ->
  ...
  | BinOpLv (bop,lv1,lv2) -> constp_bop bop (constp_aexp lv1 cmem) (constp_aexp lv2 cmem)
  | AHole _ -> ConstTop
...
= fun aexp cmem ->
  ...
  | _ -> (* partial *)
   (match aexp with 
    | BinOpLv (bop,lv1,lv2) -> 
      (match constp_aexp lv1 cmem, constp_aexp lv2 cmem with
       | Const n1, _ -> BinOpLv (bop,lv2, Int n1)
       | _, Const n2 -> BinOpLv (bop,lv1, Int n2)
       | _ -> aexp)
    | _ -> aexp)
...
= fun bexp cmem -> 
  ...
  | Gt (lv1,lv2) -> 
    (match constp_aexp lv1 cmem, constp_aexp lv2 cmem with
     ...
     | _,Const n2 -> Gt (lv1, Int n2)
     | _ -> bexp)
  | Lt (lv1,lv2) ->
    (match constp_aexp lv1 cmem, constp_aexp lv2 cmem with
     | Const n1, Const n2 -> if n1<n2 then True else False
     | _,Const n2 -> Lt (lv1, Int n2)
     | _ -> bexp)
  | Eq (lv1,lv2) ->
    (match constp_aexp lv1 cmem, constp_aexp lv2 cmem with
     | Const n1, Const n2 -> if n1=n2 then True else False
     | _,Const n2 -> Eq (lv1, Int n2)
     | _ -> bexp)
...
let constant_propagation : lv list -> prog -> prog
...
= fun aexp cpmem ->
  ...
  | _ -> (* partial *)
    (match aexp with
     | BinOpLv (bop,lv1,lv2) -> BinOpLv (bop, trans_copyp_aexp lv1 cpmem, trans_copyp_aexp lv2 cpmem)
     | _ -> aexp)
...
= fun bexp cpmem ->
  match bexp with
  | Gt (lv1,lv2) -> Gt (trans_copyp_aexp lv1 cpmem, trans_copyp_aexp lv2 cpmem)
  | Lt (lv1,lv2) -> Lt (trans_copyp_aexp lv1 cpmem, trans_copyp_aexp lv2 cpmem)
  | Eq (lv1,lv2) -> Eq (trans_copyp_aexp lv1 cpmem, trans_copyp_aexp lv2 cpmem)
...
= fun lv_comps aexp ->
  ...
  | Int n -> BatSet.empty
  | Lv lv -> uses_lv lv
  | BinOpLv (bop,lv1,lv2) -> BatSet.union (uses_aexp lv1) (uses_aexp lv2)
...
= fun lv_comps bexp ->
  ...
  | Gt (lv1,lv2) 
  | Lt (lv1,lv2) 
  | Eq (lv1,lv2) -> BatSet.union (uses_lv lv1) (uses_lv lv2)
