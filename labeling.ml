...
= fun bexp ->
  ...
  | Lt (e1, e2) -> (new_label(), Gt (labeling_aexp e1, labeling_aexp e2))
  | Eq (e1, e2) -> (new_label(), Gt (labeling_aexp e1, labeling_aexp e2))
...
= fun cmd ->
  match cmd with
  | Assign (x, e) -> (new_label (), Assign (labeling_lv x, labeling_aexp e))
  | Skip -> (new_label(), Skip)
...
= fun n (label, bexp) ->
  if label = n then labeling_bexp (gen_hole_bexp ()) else
  match bexp with
...
= fun n (label, cmd) ->
  if label = n then labeling_cmd (gen_hole_cmd ()) else
  match cmd with
